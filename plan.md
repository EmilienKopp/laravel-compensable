# Sequence Layer — Brainstorm & Package Idea
...
## The Triality: Action / UseCase / Sequence
...
Sequence  →  sequences UseCases (or Actions) via a Pipeline. Owns the error boundary.
```
...
The discriminator between two similar sequences is one or more conditional steps (`skippable([...], callable $predicate)`, chained in order), not a boolean parameter.
...
Each Sequence defines a typed `Payload` — essentially a DTO that travels through the pipeline:
...
This makes everything more **declarative**: the Sequence declares what it needs, and the Step declares what it requires. The pipeline enforces the contract.
...
The moment a Sequence sequences DB writes alongside external API calls, no single transaction can provide consistency. DB transactions are local; API calls are not rollbackable.

This means:

- **Each UseCase owns a small, tight transaction** around its own DB write(s).
- **The Sequence owns the error boundary** (try/catch), not a transaction.
...
The only realistic recovery mechanism is **idempotency**: every Step must be safe to re-execute with the same input, because the queue layer may retry the entire Sequence.
...
{
    public function handle(SequencePayload $payload): void;
    public function rollback(SequencePayload $payload): void;
}
...
The Sequence (or Sequence) tracks which steps succeeded. On failure, it walks back in reverse and calls `rollback()` on each completed step.
...
What's described above is a **choreography saga** with optional compensation. **Temporal.io** formalizes this pattern with durable execution — Steps become Activities, the Sequence becomes a durable function that survives process restarts. The PHP package would be the lightweight, synchronous (or queue-backed) version of the same mental model.

---

## Package Sketch

**Name ideas:** `laravel-sequence`, `pipeline-saga`, `application-sequence`

**Core primitives:**

| Class / Interface      | Responsibility                                                |
| ---------------------- | ------------------------------------------------------------- |
| `Sequence`             | Base class. Declares `steps()` and `run()`.                   |
| `SequencePayload`      | Base typed payload. `set/get/has` for dynamic cases.          |
| `Sequence`     | Fluent pipeline wrapper. `->steps()->skippable()->run()`.     |
...
| `CompensatingPipeline` | Sequence variant that tracks and reverses on failure. |

**Open questions:**

- Should `Sequence` be the transaction boundary for pure-DB sequences (no external calls)?
...
- Laravel-specific: integrate with `ShouldQueue` on the Sequence itself, not individual Steps?

---

## Naming Alternatives Worth Considering

The Action/UseCase/Sequence names carry baggage from different communities. Alternatives:
...
| Sequence | Saga / Process Manager | —               | Domain Service (stretched) |
...
`Sequence` owns detection, not reaction. When a `rollback()` fails, the package logs the failure, persists a `FailedCompensation` record with full context (Sequence, Step, Payload, original exception, compensation exception, timestamp), and attempts a fixed number of retries via a dedicated queue. After the retry cap is exhausted, a hook is invoked:
